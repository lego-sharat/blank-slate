...
export default function NoteEditor() {
  ...
  const changeStatus = (status: NoteStatus) => {
    if (note) {
      note.status = status;
      note.updatedAt = Date.now();
      notes.value = [...notes.value];
  ...
  return (
    <div class="note-editor">
      <div class="note-editor-toolbar">
        <div class="note-status-buttons">
          <button
            class={`status-btn ${note.status === 'draft' ? 'active' : ''}`}
            onClick={() => changeStatus('draft')}
          >
            Draft
          </button>
          <button
            class={`status-btn ${note.status === 'ready' ? 'active' : ''}`}
        ...
        <div class="note-actions">
          ...
          <button class="icon-btn" onClick={copyMarkdown} title="Copy Markdown">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
              <rect x="9" y="9" width="13" height="13" rx="2" ry="2"/>
              <path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/>
            </svg>
          </button>
          <button class="icon-btn" onClick={deleteNote} title="Delete Note">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
